...
- 2026-03-06: Local QA seed data uses stable `QA-CLM-*` claim numbers and event-note markers so the seeding script can upsert and fully resync the Milestone 2 regression dataset over the local API without blindly duplicating projects.
- 2026-03-06: The frontend refresh is using Tailwind Plus application-shell, stats, tab, and table patterns as the default UI language, but it is being adapted selectively around the existing React/D1 workflows instead of trying to port entire example screens verbatim.
- 2026-03-06: The localhost sign-in screen now hides Google Identity Services and shows the seeded local admin credentials because the production Google client ID is not authorized for local Pages/Vite origins and was creating avoidable console noise during QA.
- 2026-03-06: Milestone 3 communication support is implemented as a manual D1-backed project communication log (email/call/text/meeting with follow-up dates) rather than a live mailbox sync, so the operations workflow is usable now without blocking on Gmail integration.
- 2026-08-16: The HUD projection uses the existing project/task/communication/invoice-event services and the dashboard's shared deterministic priority function. The endpoint has a dedicated `HUD_CLAIMS_TOKEN` secret and is not a claims-dashboard user session.
- 2026-08-16: A successful origin read receives a new timestamp-bound watermark; `dataUpdatedAt` remains the newest underlying record time. This lets the relay distinguish a fresh authoritative read from replay of an old response.
- 2026-08-16: The locked Wrangler version reported the checked-in generated binding types as stale, so `worker-configuration.d.ts` was regenerated before feature verification.
